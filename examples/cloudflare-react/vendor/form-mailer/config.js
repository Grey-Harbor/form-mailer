...
function parseLegacyRecipientMap(value) {
    if (!value) {
        return undefined;
    }
    const map = {};
...
function normalizeRecipientMapValue(value) {
...
function parseRecipientMapObject(value) {
...
function parseRecipientMapEnv(env) {
...
function parseQuotedDotenvValue(value, quote) {
...
function parseDotenvFile(source) {
    const env = {};
...
function warnIfSecretIsInEnvFile(fileEnv) {
    for (const secretName of ['FORM_MAILER_SMTP_PASSWORD', 'FORM_MAILER_SMTP_TOKEN', 'FORM_MAILER_HTTP_TOKEN']) {
        if (fileEnv[secretName]) {
            console.warn(`form-mailer: ${secretName} was loaded from FORM_MAILER_ENV_PATH. ` +
                'This is a security risk; prefer supplying secrets through the live environment instead.');
        }
    }
}
function mergeEnvironments(fileEnv, runtimeEnv) {
...
function buildConfigFromEnv(env) {
    ...
    if (httpUrl && smtpHost) {
        throw createFormMailerError('config_error', 'FORM_MAILER_HTTP_URL and FORM_MAILER_SMTP_HOST cannot both be set. Choose one built-in transport.');
    }
    return {
...
export async function loadConfigFromEnv(env = process.env) {
...
export function createTransportFromConfig(config) {
    if (config.transport) {
    ...
    if (!config.smtp) {
        throw createFormMailerError('config_error', 'SMTP or HTTP configuration is required when no transport is provided.');
    }
